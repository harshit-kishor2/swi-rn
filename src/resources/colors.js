export const PRIMARY = '#0275d8';
export const SECONDARY = '#767676';
export const WHITE = '#FFFFFF';
export const BLACK = '#000000';
export const ORANGE = '#FBA928';
export const YELLOW = '#F6D060';
export const RED = '#DF0000';
export const TRANSPARENT = 'transparent';
export const SEPARATOR_LINE_COLOR = '#ECECEC';
export const APPGREEN = '#00958C';

// ACTIONS
export const SUCCESS = '#3adb76';
export const INFO = '#5bc0de';
export const WARNING = '#ffae00';
export const ALERT = '#cc4b37';
export const DANGER = '#d9534f';

// BACKGROUND
export const BACKGROUND = '#FFFFFF';
export const BACKGROUND2 = '#F1F1F1';
export const SEPERATOR_LINE = '#716F6F';

//BOTTOM TAB BACKGROUND
export const BOTTOMTABBACKGROUND = '#FFFFFF';

//MY PROFILE
export const OPACITYCOLOR = '#5E5E5E';
export const MY_PROFILE_TAB_COLOR = '#3FE0D0';
export const SEPERATOR = '#2C2C2E';

//PREFERENCE Screen COLOR
export const LOW_BORDER_COLOR = '#595959';

//EXPLORE TAB COLOR
export const EXPLORE_TAB_FAVOURITE_COLOR = '#575757';

// TEXT COLORS
export const TEXT = '#000000';
export const LIGHT_BACKGROUND = '#4D4D4D';
export const EMAIL_TEXT = '#FEFEFE';
export const GENDER_TEXT_COLOR = '#606060';
export const HEADER_TEXT = '#00958C';

//Setting Screen
export const TRACK_BACKGROUND_COLOR = '#6A6A6A';

//Notification Screen
export const NOTIFICATION_TEXT = '#8D8D8D';
export const UN_READ_DOT_COLOR = '#F84660';
//

//  WalkThrough Screen

export const HYPERLINK = '#00958C';

// Post Product

export const PageBackground = '#F0F2FA';
export const greyTextColor = '#636363';
export const themeColor = '#00958C';
export const borderBottomColor = '#7C7C7C';
